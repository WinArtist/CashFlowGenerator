...
class ReportGenerator:
    ...
    def _build_rows(self, report_data: ReportData) -> List[List]:
        ...
        # 标题
        rows.append([f'{self.config.data.report_year}年资金收支季度报表', '', '', '', '', '', '', '', '', ''])
        rows.append(['', '', '', '', '', '', '', '', '', ''])
        
        # 表头
        rows.append(['项目', '年度合计', 'Q1', 'Q2', 'Q3', 'Q4', '', '占比', '', ''])
        rows.append(['期初数：', '0', '0', '0', '', '', '', '', '', ''])
        
        # 收入部分
        self._add_income_rows(rows, report_data)
        
        # 支出部分
        self._add_expense_rows(rows, report_data)
        
        # 汇总行
        rows.append(['本月资金净流入', 
                    f"{report_data.net_flow:.{self.decimal_places}f}",
                    f"{report_data.total_income:.{self.decimal_places}f}", 
                    '', '', '', '', '', '', ''])
        rows.append(['本月资金余额', 
                    f"{report_data.net_flow:.{self.decimal_places}f}",
                    f"{report_data.total_income:.{self.decimal_places}f}", 
    ...
    def _add_income_rows(self, rows: List[List], report_data: ReportData):
        """添加收入行"""
        ...
        for category in ['产品收入', '服务收入', '其他收入']:
            ...
            q2_amount = report_data.quarterly['Q2'].income.get(category, Decimal(0))
            rows.append([category, 
                        f"{amount:.{self.decimal_places}f}",
                        f"{q2_amount:.{self.decimal_places}f}", 
                        '', '', '', '', '', '', ''])
    ...
    def _add_expense_rows(self, rows: List[List], report_data: ReportData):
        """添加支出行"""
        # 主营业务支出
        ...
        for category in ['商品采购', '运费', '服务费', '返点佣金']:
            ...
            q2_amount = report_data.quarterly['Q2'].expense.get(category, Decimal(0))
        ...
        rows.append(['手续费', f"{report_data.expense.get('财务_手续费', Decimal(0)):.{self.decimal_places}f}", 
                    f"{report_data.quarterly['Q2'].expense.get('财务_手续费', Decimal(0)):.{self.decimal_places}f}", 
                    '', '', '', '', '', '', ''])
        ...
        rows.append(['代扣代缴个税', f"{report_data.expense.get('税金_个税', Decimal(0)):.{self.decimal_places}f}", 
                    f"{report_data.quarterly['Q2'].expense.get('税金_个税', Decimal(0)):.{self.decimal_places}f}", 
                    '', '', '', '', '', '', ''])
    ...
    def _write_summary_sheet(self, writer: pd.ExcelWriter, report_data: ReportData):
        ...
        summary.append(['', '', ''])
        summary.append(['收入类别', '年度合计(元)', 'Q2金额(元)'])
        
        for category in ['产品收入', '服务收入', '其他收入']:
            amount = report_data.income.get(category, Decimal(0))
            q2_amount = report_data.quarterly['Q2'].income.get(category, Decimal(0))
            summary.append([category, f"{amount:.2f}", f"{q2_amount:.2f}"])
        
        summary.append(['收入合计', f"{report_data.total_income:.2f}", f"{report_data.total_income:.2f}"])
        summary.append(['', '', ''])
        summary.append(['支出类别', '年度合计(元)', 'Q2金额(元)'])
        ...
        for category in expense_order:
            ...
            if amount > 0:
                ...
                summary.append([display_name, f"{amount:.2f}", f"{q2_amount:.2f}"])
        
        summary.append(['支出合计', f"{report_data.total_expense:.2f}", f"{report_data.total_expense:.2f}"])
        summary.append(['', '', ''])
        summary.append(['净现金流', f"{report_data.net_flow:.2f}", f"{report_data.net_flow:.2f}"])
    ...
    def _write_detail_sheet(self, writer: pd.ExcelWriter, report_data: ReportData):
        ...
        for category, amount in sorted(report_data.expense.items(), key=lambda x: -x[1]):
            if amount > 0:
                ...
                details.append([display_name, f"{amount:.2f}", ''])
